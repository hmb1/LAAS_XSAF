    ...
    integer, parameter :: max_transfer_redshifts = 3000
...
500     if (L< lmax_extrap_highl) &
    ...
    do in=1, CP%InitPower%nn
        ...
        do j_PK=1, CP%Transfer%PK_num_redshifts
            ...
            !write(*,'("at z =",f7.3," sigma8 (all matter) = ",f12.10)') &
    ...
    do itf=1, CP%Transfer%PK_num_redshifts
        if (FileNames(itf) /= '') then
            if (.not. transfer_interp_matterpower ) then
                ...
                do i=1,points
                    write (unit, '(*(E30.12))') MTrans%TransferData(Transfer_kh,i,1),outpower(i,1:CP%InitPower%nn,:)
                end do
                close(unit)
            else
                minkh = 1e-4
                ...
                do i=1,points
                    write (unit, '(*(E30.12))') minkh*exp((i-1)*dlnkh),outpower(i,1:CP%InitPower%nn,1)
                end do
                close(unit)
            end if
