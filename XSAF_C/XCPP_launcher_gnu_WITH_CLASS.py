...
zcut = 'N'
# Compute WP_Pk
# 3 options : seq, par or regex(seq)
Powspec_choice = ['Y','seq']
# Compute Photo-z
photoZ = 'Y'
# Compute C_l's
compute_Cl = 'Y'
...
def Using_WP_Pk(Powspec_choice):
  if (Powspec_choice[0] == 'Y'):
    print('Removing current WP_Pk CLASS')
    # Create output directory
    if not os.path.exists("output"):
      os.makedirs("output")
    if not os.path.exists('WP_Pk'):
      os.makedirs('./WP_Pk')
    # Clean current WP_Pk
    os.system('rm -rf ./WP_Pk/*')
    os.chdir('CLASS')
    if (Powspec_choice[1] == 'seq'):
      # Sequential
      print('Calling CLASS - regex(seq)')
      if not os.path.exists('WP_Pk_seq_regex'):
        os.makedirs('./WP_Pk_seq_regex')
      os.system('rm -rf ./WP_Pk_seq_regex/*')
      os.system('python Class_launcher_XSAF_regex.py')
      os.system('cp -rf ./WP_Pk_seq_regex/* DIR_XSAF_C/WP_Pk/')
      print('CLASS COMPUTED')
    else:
      print('Error on mode for Power spectrum CLASS')
    # Return to XSAF_C directory
    os.chdir('DIR_XSAF_C/')
